...
def first_number_in_text(line, dict_numbers):
    for index in range(len(line)):
        for length in range(1, len(line) - index + 1):
            substring = line[index: index + length]
            if substring in dict_numbers:
                return dict_numbers[substring]
            if substring.isdigit():
                return substring
    return None

def last_number_in_text(line, dict_numbers):
    for index in range(len(line), -1, -1):
        for length in range(index, -1, -1):
            substring = line[length:index]
            if substring in dict_numbers:
                return dict_numbers[substring]
            if substring.isdigit():
                return substring
...
def calculate_calibration_values(input_text):
    lines = re.findall(r'[a-z0-9]+', input_text)
    word_number_dict = {
        "one": "1",
        "two": "2",
        "three": "3",
    ...
    total = 0
    for line in lines:
        first_num = first_number_in_text(line, word_number_dict)
        last_num = last_number_in_text(line, word_number_dict)
...
"""
print(calculate_calibration_values(input_text))