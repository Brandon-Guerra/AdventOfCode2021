...
const practice = `twone`;

const x = practice
  .split("\n")
  .map((line) => {
    const numbers = stringParser(line).replace(/\D/g, "");
    console.log(numbers);
    const s = parseInt("" + numbers[0] + numbers[numbers.length - 1]);
    console.log(s);
    return s;
  })
...
function stringParser(x: string): string {
  ...
  values.push([one, x.indexOf(one)]);
  values.push([two, x.indexOf(two)]);
  values.push([three, x.indexOf(three)]);
  values.push([four, x.indexOf(four)]);
  values.push([five, x.indexOf(five)]);
  values.push([six, x.indexOf(six)]);
  values.push([seven, x.indexOf(seven)]);
  values.push([eight, x.indexOf(eight)]);
  values.push([nine, x.indexOf(nine)]);
  ...
  for (const value of sorted) {
    finalString = finalString.replaceAll(value, valueMap[value] + value);
  }
