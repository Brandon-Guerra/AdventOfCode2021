...
def is_group_valid(amount, color)
    is_valid = true
    red_count = 12
    green_count = 13
    blue_count = 14
...
input.split("\n").map { |game|
    ...
    game_id = components[0].split(" ")[1]
    sets = components[1]
    sets.split(";").map { |set| set.split(",").map { |color_set|
        amount = color_set.split(" ")[0].to_i
        color = color_set.split(" ")[1]
        case color
        when "red"
            reds.push(amount)
        when "green"
    ...
    red_power = reds.map { |red| red.to_i }.max
    green_power = greens.map { |green| green.to_i }.max
    blue_power = blues.map { |blue| blue.to_i }.max

    game_powers.push(red_power * green_power * blue_power)
    is_valid_game = sets.split(";").map { |set| set.split(",").map { |color_set| is_group_valid(color_set.split(" ")[0].to_i, color_set.split(" ")[1])}}.flatten.all?
    if is_valid_game
        valid_game_ids.push(game_id)
    end
}

puts valid_game_ids.map { |game_id| game_id.to_i }.sum
