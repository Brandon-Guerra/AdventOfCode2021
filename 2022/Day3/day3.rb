...
commons = rucksacks.map { |rucksack| rucksack[0..rucksack.length() / 2 - 1].split("").to_set & rucksack[rucksack.length() / 2..rucksack.length - 1].split("").to_set }
part_one = commons.map { |common| priorities.find_index(common.to_a[0]) }.sum

print part_one, "\n"

duplicates = rucksacks.each_slice(3).to_a.map { |set_of_3| set_of_3[0].split("").to_set & set_of_3[1].split("").to_set & set_of_3[2].split("").to_set }
part_two = duplicates.map { |common| priorities.find_index(common.to_a[0]) }.sum
